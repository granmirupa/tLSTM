...
class tLSTMv2(nn.Module):
    ...
    def forward(self, input, labels, time, ctx = None, ctx_mask=None):   
        def recurrence(input, labels_prev, time, hidden):
            ...
            #Gates 
            ingate = F.sigmoid(self.weight_ix(features) + self.weight_im(labels_prev))
            forgetgate = F.sigmoid(self.weight_fx(features) + self.weight_fm(labels_prev))
            cellgate = F.tanh(self.weight_cx(features) + self.weight_cm(labels_prev))
            outgate = F.sigmoid(self.weight_ox(features) + self.weight_om(labels_prev))
            
            cy = (forgetgate * cx) + (ingate * cellgate) + self.weight_ft(time)
